...
function App() {
  ...
  const req = async() => {
    await axios.get(`https://deviant-girls.online/girls/anna-delos${searchParams.get('app') !== null ? "?app=" : ""}`)
    .then((res) => {
      setGirl(res.data)
        if(res.status === 200){
  ...
  return (
    ...
    <div className="clouds"></div>
        ...
        <div className="flex flex-col mb-4 ">
          <p className="text-4xl max-md:text-6xl text-center pt-2 tracking-widest z-20">{girl.nickname}</p>
            <div className=" flex items-center justify-center">
                <div className="blob green max-md:mt-1"></div>
                <p className='ml-1 max-md:text-xl tracking-widest  z-20'>online</p>
            </div>
        </div>
                      ...
                      <svg width="30" height="30" viewBox="0 0 126 126" fill="" xmlns="http://www.w3.org/2000/svg" className='absolute bottom-36 right-12 max-md:bottom-48 max-md:right-16'>
                            <defs>  
                              <linearGradient id="logo-gradient" x1="0%" y1="0%" x2="100%" y2="0%" > 
                                  <stop offset="0%" stopColor="#A292FF">
                                      <animate attributeName="stop-color" values="#D7AFFF; #D7C3FF; #A292FF" dur="4s" repeatCount="indefinite"></animate>
                                  </stop>

                                  <stop offset="50%" stopColor="#C9AFFF">
                                      <animate attributeName="stop-color" values="#C9AFFF; #D7C3FF; #D7AFFF" dur="4s" repeatCount="indefinite"></animate>
                                  </stop>

                                  <stop offset="100%" stopColor="#D7C3FF">
                                      <animate attributeName="stop-color" values="#D7C3FF; #E4D8FF; #D7C3FF" dur="4s" repeatCount="indefinite"></animate>
                                  </stop>
                              </linearGradient> 
                          </defs>
      ...
      <p className=' text-lg mt-2 mb-2 h-full max-md:text-2xl max-s:w-80 max-sm:w-96 max-md:mt-4 max-md:mb-4 text-center tracking-widest'>
